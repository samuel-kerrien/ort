...
class FreemarkerTemplateProcessor(
    ...
    fun processTemplates(input: ReporterInput, outputDir: File, options: Map<String, String>): List<File> {
        ...
        if (projectTypesAsPackages.isNotEmpty()) {
            log.info { "Handling ${projectTypesAsPackages.size} of types $projectTypesAsPackages as packages." }
        }
 ...
 * projects not in [targetProjects].
 */
private fun OrtResult.deduplicateProjectScanResults(targetProjects: Set<Identifier>): OrtResult {
